...
results_dir <- here::here("18s/results")
...
```{r}
samples_df <- read.table(here("18s/data/metadata/samples.tsv"), header=TRUE, stringsAsFactors=FALSE)
...
```{r}
primers_filename <- here("18s/ref_18s/primers.tsv")
suppressMessages(
...
```{r}
metadata_filename <- here("18s/ref_18s/metadata.tsv")
suppressMessages(
...
```{r}
mkdir(here("18s/cache"))
save.image(here("18s/cache/01.RData"))
```
