...
load(here::here("18s/cache/01.RData"))
```
...
truncation_lengths <- data.frame(
  SampleName = c("g18s"),
  forward_trunc_length = c(220),
  reverse_trunc_length = c(220)
)
...
# targets <- targets %>% filter(SampleName == "g18s")
targets <- targets %>% filter(SampleName %in% c("g18s"))
...
```{r}
paged_table(read_qza(here("18s/results/dada2/g18s_denoising_stats.qza"))$data)
```
...
```{r}
g18s_table <- read_qza(here("18s/results/dada2/g18s_table.qza"))
g18s_table_df <- data.frame(g18s_table$data)
g18s_match_list <- read.table(here("18s/results/lulu/g18s_match_list.txt"))

# TODO: Currently using default parameters
z <- capture.output(g18s_lulu <- lulu(g18s_table_df, g18s_match_list))

dim(g18s_lulu$original_table)
dim(g18s_lulu$curated_table)
```

```{r}
saveRDS(g18s_lulu, file=here("18s/results/lulu/18s_lulu.rds"))
```
...
```{r}
save.image(here("18s/cache/02.RData"))
```
