...
load(here::here("18s/cache/02.RData"))
```
...
```{r}
ref_db_dir <- "~/Documents/DNA_reference_db_construction_final_libraries/final_library"

db_18s_ids=list(
  g18s=file.path(ref_db_dir, "18s_db_2024-06.txt")
)

# FASTA file with COI sequences
db_fasta_files=list(g18s=file.path(ref_db_dir, "18s_db_2024-06.fasta")
)
```

```{r}
taxonomy_ref <- data.frame(SampleName=names(db_fasta_files),
                           db_18s_id_file=unlist(db_18s_ids[names(db_fasta_files)]),
...
targets <- data.frame(FileName=taxonomy_ref$db_fasta_file,
                      taxonomy_id_file=taxonomy_ref$db_18s_id_file,
...
write_targets(targets %>% filter(SampleName %in% c("g18s")), file=database_targets)
```
...
if (! big_dataset) {
  ...
  write_targets(targets %>% filter(amplicon_group %in% c("g18s")), file=prelim_classification_targets)
}
```
...
```{r}
get_thresholds <- function(file_path, offset=1, max_threshold=80) {
...
threshold_file_list <- data.frame(
  amplicon_group = c("g18s"),
  file_list = c(here("18s/results/classification/thresholds/g18s_files.txt"))
)
```
...
# WIP: For the moment, only run the g18st amplicions
write_targets(targets %>% filter(amplicon_group %in% c("g18s")), file=classification_targets)
```
...
```{r}
save.image(here("18s/cache/03.RData"))
```
