...
=======
>>>>>>> 7dce5db224d7c8f05a33bcbbf65e91f6742a5bd6
...
```{r}
# change slurm setting for large datasets
use_slurm <- TRUE
slurm_threads <- 2
slurm_resources <- list(partition="main", ntasks=1, ncpus=slurm_threads, memory=16384)
slurm_n_jobs <- 2
```
...
Starting from here will need the thershold list from Cache03
```{r}
...
```{r}
# bind short and right classificarion data
...
# all_readclass_df <- merge(all_classif_df, all_read_df = "asv_code", all.x = TRUE)
...
# save dataframe
write.csv(final_df, file = "~/git/metabarcoding_workflow/results/final_data_summary.csv", row.names = FALSE)
