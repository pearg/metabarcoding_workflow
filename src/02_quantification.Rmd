...
```{r}
before_filtering <- read_qza(here("results/dada2/short_table.qza"))
after_filtering <- read_qza(here("results/dada2/short_table_filtered.qza"))
```
...
```{r}
short_table <- read_qza(here("results/dada2/short_table.qza"))
short_table_df <- data.frame(short_table$data)
short_match_list <- read.table(here("results/lulu/short_match_list.txt"))
...
```{r}
saveRDS(short_lulu, file=here("results/lulu/short_lulu.rds"))
```


```{r}
right_table <- read_qza(here("results/dada2/right_table.qza"))
right_table_df <- data.frame(right_table$data)
right_match_list <- read.table(here("results/lulu/right_match_list.txt"))
...
```{r}
saveRDS(right_lulu, file=here("results/lulu/right_lulu.rds"))
```
