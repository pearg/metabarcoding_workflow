...
cont_columns <- grepl("Cont", colnames(all_tables[[l]]))

# Subset the data frame to exclude columns with 'Cont' in their names
filtered_all_table_sample <- all_tables[[l]][, !cont_columns]
...
```{r}
# create a list of values that group all samples from the same site. Only required for the percentage sample set
site_sample <- lapply(sample_groups, function(x) {
  if (!grepl("^Cont", x) && grepl("\\d", x)) {
...
# convert kist to dataframe
site_sample <- do.call(data.frame, site_sample)
colnames(site_sample) <- gsub("^X", "", colnames(site_sample))
```
...
for (l in names(all_tables)) {
  matrix_list <- list()
  for (g in site_sample) {
    sample_ids <- names(site_sample)[site_sample == g]
    x <- all_tables[[l]][,sample_ids]  # Use double square brackets and drop = FALSE
    ok1 <- x > 0
...
l <- "10BNY29042rep1"  # Replace with an actual table name from all_tables
sample_ids <- c("10BNY29042rep1", "10BNY29042rep2")  # Replace with actual column names
print(colnames(all_tables[[l]]))
print(sample_ids)
x <- all_tables[[l]][, sample_ids, drop = FALSE]
```
