...
# to metabarcoding workflow directory
# metawork <- "~/git/metabarcoding_workflow/"
...
```{r}
samples_df <- read.table(here("plants/data/metadata/samples.tsv"), header=TRUE, stringsAsFactors=FALSE)
...
```{r}
primers_filename <- here("plants/ref_plant/primers.tsv")
suppressMessages(
...
```{r}
metadata_filename <- here("plants/ref_plant/metadata.tsv")
suppressMessages(
...
```{r}
mkdir(here("plants/cache"))
save.image(here("plants/cache/01.RData"))
```
