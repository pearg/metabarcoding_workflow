...
truncation_lengths <- data.frame(
  ...
  forward_trunc_length = c(150, 140, 40),
  reverse_trunc_length = c(150, 140, 40)
)
...
```{r}
before_filtering <- read_qza(file.path(dada2_dir, "trnl_table.qza"))
after_filtering <- read_qza(file.path(dada2_dir, "trnl_table_filtered.qza"))
```
...
```{r}
save.image(here("plants/cache_trnl/02.RData"))
```
