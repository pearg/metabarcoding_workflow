...
load(here::here("plants/cache_trnl/02.RData"))
```
...
db_trnl_ids=list(trnl=file.path(ref_db_dir, "trnl_db_2024-05.txt"))

# FASTA file with trnl sequences
db_fasta_files=list(
  trnl=file.path(ref_db_dir, "trnl_db_2024-05.fasta"))
```

```{r}
taxonomy_ref <- data.frame(SampleName=names(db_fasta_files),
                           db_trnl_id_file=unlist(db_trnl_ids[names(db_fasta_files)]),
...
targets <- data.frame(FileName=taxonomy_ref$db_fasta_file,
                      taxonomy_id_file=taxonomy_ref$db_trnl_id_file,
...
write_targets(targets %>% filter(SampleName %in% c("trnl")), file=database_targets)
```
...
if (! big_dataset) {
  ...
  write_targets(targets %>% filter(amplicon_group %in% c("trnl")), file=prelim_classification_targets)
}
```
...
threshold_file_list <- data.frame(
  amplicon_group=c("trnl"),
  file_list=c(here("plants/results/classification/thresholds/trnl_files.txt"))
)
paged_table(threshold_file_list)

# save threshold list
saveRDS(thresholds_list, file = "~/git/metabarcoding_workflow/plants/results_trnl/thresholds_list_trnl.rds") 
```
...
# WIP: For the moment, only run the short and right amplicions
write_targets(targets %>% filter(amplicon_group %in% c("trnl")), file=classification_targets)
```
