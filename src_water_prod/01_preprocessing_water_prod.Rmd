---
title: "metabarcoding_preprocessing"
...
#***NOTE: 'test' will need to be change to your miseq folder name****

results_dir <- "/mnt/uomShare/wergStaff/MelCarew/git-data/miseq18/metabarcoding_workflow"

# Set the directory path for files to be stored on uomshare (NOTE: 'test' will need to be change to your miseq folder name)
uomshare <- "/mnt/uomShare/wergStaff/MelCarew/git-data/miseq18/metabarcoding_workflow/"
...
We also have the option to run the jobs in this workflow using SLURM if you're working on a cluster that uses it.
...
```{r}
samples_df <- read.table(paste0(uomshare,"data/metadata/samples.tsv"), header=TRUE, stringsAsFactors=FALSE)
...
To import data into QIIME, the FASTQ files for each library need to be in separate directories. First, let's check that all files exist from the `samples.tsv` file provided.
...
If all the files exist, we can now create a separate directory for each sample have changed this -link back to the original FASTQ files using symbolic links (shortcuts). Please note Mel has changed this for uomshare to not use symlinks

```{r}
# Create some directories first
dir.create(results_dir)
dir.create(logs_dir)
dir.create(paste0(uomshare, "tmp"))

# Create import_data directory if it doesn't exist
import_data_dir <- file.path(results_dir, "import_data")
if (!dir.exists(import_data_dir)) {
  dir.create(import_data_dir, recursive = TRUE)
}
...
for (i in seq_len(nrow(samples_df))) {
  ...
  if (!file.exists(file.path(lib_dir, "forward.fastq.gz"))) {
    file.copy(from = file.path(here(x$filename_r1)), 
              to = file.path(lib_dir, "forward.fastq.gz"))
  }
  
  if (!file.exists(file.path(lib_dir, "reverse.fastq.gz"))) {
    file.copy(from = file.path(here(x$filename_r2)), 
...
import_targets <- paste0(uomshare, "tmp/import_targets.txt")
...
demux_targets <- paste0(uomshare,"tmp/demux_targets.txt")
...
split_targets <- paste0(uomshare,"tmp/split_targets.txt")
...
trimmed_targets <- paste0(uomshare,"tmp/trimmed_targets.txt")
...
summary_targets <- paste0(uomshare,"tmp/summary_targets.txt")
...
If you want to view an individual sample's sequence base quality boxplot, you can view the sample's qzv file located in: **`r summary_dir`**\
...
```{r}
mkdir(paste0(uomshare,"cache"))
save.image(paste0(uomshare,"cache/01.RData"))
```
