...
```{r}
# import rarefaction data from the '.qzv' file
right_rf30 <- read_csv("~/git/metabarcoding_workflow/results/rarefaction/right_mult_rarefaction_100_to_30K_by_sample/observed_features.csv")
...
ggplot(data = right_rf30_table_av, aes(x = depth, y = avg_ASV_no, group = sample_id, color = sample_id)) +
  geom_line(show.legend = FALSE) +
  labs(title = "right amplicon alpha rarefraction at 30K sequencing depth",
       x = "sequencing depth",
       y = "number of ASVs",
       color = "Sample ID") +
  theme_minimal()
